...
/** Le ticket qui « sort de l'imprimante » : la clé, groupée par 5 caractères. */
  ...
  <div className="ticket-wrap" ref={ref}>
    <div className="ticket print">
      <div className="ticket-head">
        ...
        <span>{formatDate(entry.iat, 'short')}</span>
      </div>
      <div className="ticket-body">
        <p className="key" aria-label={`Clé d'activation : ${entry.key}`}>
          {entry.key.split('-').map((group, i) => (
            <span className="g" key={i}>{group}</span>
          ))}
        </p>
      </div>
    </div>
