...
export default function History({ entries, onRemove }: Props) {
  ...
  if (entries.length === 0) {
    return <p className="empty-state">Aucune clé pour l'instant. Générez la première depuis l'onglet Générer.</p>;
  }
  ...
  return (
    <>
      ...
      <IonList lines="none" className="h-list">
        {entries.map((e) => (
          <IonItemSliding key={e.mid + e.iat}>
            <IonItem lines="full" className="h-item">
              <div className="h-body">
                <div className="h-top">
                  <span className={e.client ? 'h-client' : 'h-client empty'}>{e.client || 'Sans nom'}</span>
                  <span className="h-date">{formatDate(e.iat, 'short')}</span>
                </div>
                <div className="h-key">{e.key}</div>
                <div className="h-mid">Machine <code>{e.mid}</code></div>
                <div className="h-actions">
                  <IonButton
                    size="small"
                    fill="outline"
                    onClick={async () => toast({ message: (await copyText(e.key)) ? 'Clé copiée' : 'Copie impossible', duration: 1600, position: 'top' })}
                  >
                    <IonIcon slot="start" icon={copyOutline} />
                    Copier
                  </IonButton>
                  <IonButton size="small" fill="outline" onClick={() => void shareMessage(buildMessage(e.key, e.client))}>
            ...
            <IonItemOptions side="end">
              <IonItemOption color="danger" onClick={() => onRemove(e)}>Retirer</IonItemOption>
            </IonItemOptions>
          </IonItemSliding>
        ))}
      </IonList>
