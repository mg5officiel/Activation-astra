...
import {
  IonApp, IonButton, IonButtons, IonContent, IonFooter, IonHeader, IonIcon, IonLabel, IonPage,
  IonSegment, IonSegmentButton, IonTitle, IonToolbar, setupIonicReact, useIonAlert,
} from '@ionic/react';
...
const sameEntry = (a: KeyEntry, b: KeyEntry) => a.mid === b.mid && a.iat === b.iat;
...
function Shell() {
  ...
  return (
    <IonPage>
      <IonHeader>
        <IonToolbar>
          <IonTitle>{view === 'generer' ? "Clés d'activation" : 'Historique'}</IonTitle>
          {view === 'historique' && entries.length > 0 && (
            <IonButtons slot="end">
              <IonButton color="danger" onClick={askClear}>Vider</IonButton>
            </IonButtons>
          )}
        </IonToolbar>
      </IonHeader>
      ...
      <IonContent>
        <div className="sheet">
          <div hidden={view !== 'generer'}>
            <Generate onCreated={addEntry} />
          </div>
          <div hidden={view !== 'historique'}>
            <History entries={entries} onRemove={removeEntry} />
          </div>
        </div>
      </IonContent>
      ...
      <IonFooter>
        <IonToolbar>
          ...
          >
            <IonSegmentButton value="generer" layout="icon-top">
              <IonIcon icon={keyOutline} />
              <IonLabel>Générer</IonLabel>
            </IonSegmentButton>
            <IonSegmentButton value="historique" layout="icon-top">
