/**
 * Équivalent de votre crypto.ts.
 *
 * Le module Node « crypto » n'existe pas dans un téléphone : on utilise Web Crypto
 * (crypto.subtle), donc hmacSign est asynchrone ici. Quand crypto.subtle n'est pas
 * disponible (page ouverte en http pendant le développement), on bascule sur un
 * HMAC-SHA256 en JavaScript pur, qui donne exactement le même résultat.
 */
...
const SECRET = new Uint8Array(_k);

/* ── SHA-256 / HMAC en JS pur (repli) ─────────────────────────────────── */

const rotr = (x: number, n: number): number => (x >>> n) | (x << (32 - n));

const PRIMES: number[] = (() => {
  const p: number[] = [];
  for (let n = 2; p.length < 64; n++) if (p.every((q) => n % q)) p.push(n);
  return p;
})();
...
function hmacSha256(key: Uint8Array, msg: Uint8Array): Uint8Array {
  ...
  k.set(key.length > B ? sha256(key) : key);
  const inner = new Uint8Array(B + msg.length);
  for (let i = 0; i < B; i++) inner[i] = k[i] ^ 0x36;
  inner.set(msg, B);
  const outer = new Uint8Array(B + 32);
  for (let i = 0; i < B; i++) outer[i] = k[i] ^ 0x5c;
...
export async function hmacSign(data: string, forceFallback = false): Promise<string> {
  ...
  return toHex(hmacSha256(SECRET, msg));
}
